...
bot = telebot.TeleBot('Рабочая версия с user_id и message.from_user.first_name')
...
def web_app_keyboard(user_id):
    if str(user_id) in allowed_users:
        ...
        bot.send_message(user_id, '{}, у вас нет доступа.'.format(user_id))
        return None
...
def start_fun(message):
    ...
    bot.send_message(user_id, 'Привет {}, я бот для проверки телеграмм webapps!)\nЗапустить тестовые страницы можно нажав на кнопки.'.format(message.from_user.first_name), parse_mode="Markdown", reply_markup=web_app_keyboard(user_id))
